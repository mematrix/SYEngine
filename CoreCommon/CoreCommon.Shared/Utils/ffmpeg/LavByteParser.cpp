...
#include "LavByteParser.h"
#include "avcodec_get_bits.h"
#include <memory.h>

CByteParser::CByteParser(const unsigned char *pData, size_t length)
  : m_pData(pData), m_pEnd(pData+length)
{
  m_gbCtx = (GetBitContext *)malloc(sizeof(GetBitContext));
  memset(m_gbCtx,0,sizeof(GetBitContext));
...
{
  int n = -1;
  for(unsigned char b = 0; !b && RemainingBits(); n++) {
    b = get_bits1(m_gbCtx);
  }
  if (!RemainingBits())
    return 0;
