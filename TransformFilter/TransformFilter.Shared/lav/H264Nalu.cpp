...
void CH264Nalu::SetBuffer(const unsigned char* pBuffer, size_t nSize, int nNALSize/* = 0*/)
{
	m_pBuffer		= const_cast<unsigned char*>(pBuffer);
	m_nSize			= nSize;
	m_nNALSize		= nNALSize;
	m_nCurPos		= 0;
	m_nNextRTP		= 0;
...
bool CH264Nalu::MoveToNextAnnexBStartcode()
{
	...
	for (size_t i = m_nCurPos; i < nBuffEnd; i++) {
		if ((*((unsigned*)(m_pBuffer + i)) & 0x00FFFFFF) == 0x00010000) {
...
bool CH264Nalu::ReadNext()
{
	...
	if ((m_nNALSize != 0) && (m_nCurPos == m_nNextRTP)) {
		...
		while (m_pBuffer[m_nCurPos] == 0x00 && ((*((unsigned*)(m_pBuffer + m_nCurPos)) & 0x00FFFFFF) != 0x00010000)) {
			m_nCurPos++;
		}
