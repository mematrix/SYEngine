...
public :
	...
	unsigned char* GetDataBuffer() { return m_pBuffer + m_nNALDataPos; };
	...
	unsigned char* GetNALBuffer() { return m_pBuffer + m_nNALStartPos; };
	bool IsEOF() const { return m_nCurPos >= m_nSize; };

	void SetBuffer(const unsigned char* pBuffer, size_t nSize, int nNALSize = 0);
	bool ReadNext();
};
